...

            {
                categories && categories.map((item,index) => (
                    <div key={index} onClick={()=>handleClick(item)}  className='relative group overflow-hidden w-[300px]  rounded-lg'>
                        ...
                        <p className='font-semibold text-xl absolute bottom-2 left-2 group-hover:opacity-100 opacity-0 transition-all delay-150 duration-300 ease-in-out  '>{item?.category}</p>
                    </div>
                ))
            }
